...
void GrenadePrediction::View(CViewSetup* setup)
{
	...
	if (g_LocalPlayer && ((C_BasePlayer*)g_LocalPlayer)->IsAlive())
	{
		C_BaseCombatWeapon* weapon = (C_BaseCombatWeapon*)g_EntityList->GetClientEntityFromHandle(((C_BasePlayer*)g_LocalPlayer)->m_hActiveWeapon());
		if (weapon && weapon->IsGrenade() && act != ACT_NONE)
		{
			type = weapon->GetClientClass()->m_ClassID;
			Simulate(setup);
		}
		else
			type = 0;
...
void GrenadePrediction::Render()
{
	...
	if ((type) && path.size()>1)
	{
		...
		Color lineColor = g_Options.color_esp_grenade;
		Vector prev = path[0];
		...
		{
			g_VGuiSurface->DrawSetColor(lineColor.GetInverted());
...
bool GrenadePrediction::CheckDetonate(const Vector& vecThrow, const trace_t& tr, int tick, float interval)
{
	...
	default:
		//If grenade is diferent for some reason - cast it as HE.
		return static_cast<float>(tick)*interval>1.5f && !(tick%static_cast<int>(0.2f / interval));	
	}
}
