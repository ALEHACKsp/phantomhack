...
void ImGui::RenderTextTitle(const ImVec2& pos_min, const ImVec2& pos_max, const char* text, const char* text_end, const ImVec2* text_size_if_known, ImGuiAlign align, const ImVec2* clip_min, const ImVec2* clip_max) {
	...
	{//Window title render colorizer
	 /*static float h = 1.1f, s = 1.f, v = 1.f, a = 1.f;
		h += (1.f/(float)text_len) - 0.00005f;
		if (h > 1.f)
			h -= 1.f;*/
		static float _r = 1.0f, _g = 1.0f, _b = 1.0f, _a = 1.0f;
		ImU32 color = GetColorU32(ImColor(_r, _g, _b, _a));
...
bool ImGui::ButtonEx( const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags, const char* idStr)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
...
bool ImGui::Button(const char* label, const ImVec2& size_arg, const char* idStr) 
{
	return ButtonEx(label, size_arg, 0, idStr);
}
...
bool ImGui::Checkbox(const char* label, bool* v, const char* tooltip, const char* _id)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
	...
	const ImGuiStyle& style = g.Style;
	ImGuiID id = window->GetID(label);
	size_t ilen = strlen(_id);
	size_t ttlen = strlen(tooltip);
	if (ilen > 0)
		window->GetID(_id);
	else
		window->GetID(label);
...
bool ImGui::RadioButtonEx(const char* label, bool active, const char* tooltip)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
...
bool ImGui::RadioButtonEx(const char* label, int* v, int v_button, const char* tooltip)
{
	const bool pressed = RadioButtonEx(label, *v == v_button, tooltip);
	if (pressed) {
		*v = v_button;
	}
	return pressed;
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data) 
{
	...
	const bool is_nolabel = (flags & ImGuiInputTextFlags_NoLabel) != 0;
...
int _fastcall rgb_ftoi(float f)
{
	f = f * 100.f;
	f = (255.0f / 100.0f) * f; 
	int i = (int)ceil(f);
...
float _fastcall rgb_itof(int i)
{
	float f = ((i * 100.0f) / 255) * 0.01f;
	if (f > 1.00f)
		f = 1.00f;
	if (f < 0)
		f = 0;
...
bool ImGui::ColorEdit3INT(const char* label, int col[3]) 
{
	float rgba[4];
	rgba[0] = rgb_itof(col[0]);
	rgba[1] = rgb_itof(col[1]);
	rgba[2] = rgb_itof(col[2]);
	rgba[3] = 1.0f;
	const bool value_changed = ColorEdit4(label, rgba, false);
	col[0] = rgb_ftoi(rgba[0]);
	col[1] = rgb_ftoi(rgba[1]);
	col[2] = rgb_ftoi(rgba[2]);
	return value_changed;
}

bool ImGui::ColorEdit4INT(const char* label, int col[4])
{
	float rgba[4];
	rgba[0] = rgb_itof(col[0]);
	rgba[1] = rgb_itof(col[1]);
	rgba[2] = rgb_itof(col[2]);
	rgba[3] = rgb_itof(col[3]);
	const bool value_changed = ColorEdit4(label, rgba, false);
	col[0] = rgb_ftoi(rgba[0]);
	col[1] = rgb_ftoi(rgba[1]);
	col[2] = rgb_ftoi(rgba[2]);
	col[3] = rgb_ftoi(rgba[3]);
	return value_changed;
}
